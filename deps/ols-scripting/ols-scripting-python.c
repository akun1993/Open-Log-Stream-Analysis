...
static struct ols_python_script *first_tick_script = NULL;
...
bool py_to_libols_(const char *type, PyObject *py_in, void *libols_out,
		   const char *id, const char *func, int line)
{
	...
	int ret = SWIG_ConvertPtr(py_in, libols_out, info, 0);
	if (!SWIG_IsOK(ret)) {
		warn("%s:%d: SWIG failed to convert python object to ols "
		     "object: %s%s%s",
...
bool libols_to_py_(const char *type, void *libols_in, bool ownership,
		   ...
		   int line)
{
	...
	*py_out = SWIG_NewPointerObj(libols_in, info, (int)ownership);
	if (*py_out == Py_None) {
		warn("%s:%d: SWIG failed to convert ols object to python "
		     "object: %s%s%s",
		     func, line, id ? id : "", id ? "::" : "", type);
		return false;
	}
...
#define libols_to_py(type, ols_obj, ownership, py_obj)                        \
	libols_to_py_(#type " *", ols_obj, ownership, py_obj, NULL, __func__, \
		      __LINE__)
#define py_to_libols(type, py_obj, libols_out) \
	py_to_libols_(#type " *", py_obj, libols_out, NULL, __func__, __LINE__)
...
static void get_defaults(struct ols_python_script *data, PyObject *get_defs)
{
	PyObject *py_settings;
	if (!libols_to_py(ols_data_t, data->base.settings, false, &py_settings))
		return;
...
static bool load_python_script(struct ols_python_script *data)
{
	...
	if (py_tick) {
		...
		struct ols_python_script *next = first_tick_script;
		data->next_tick = next;
	...
	if (py_load) {
		PyObject *py_s;
		libols_to_py(ols_data_t, data->base.settings, false, &py_s);
...
struct python_ols_timer {
	struct python_ols_timer *next;
	struct python_ols_timer **p_prev_next;
...
static struct python_ols_timer *first_timer = NULL;

static inline void python_ols_timer_init(struct python_ols_timer *timer)
{
	pthread_mutex_lock(&timer_mutex);

	struct python_ols_timer *next = first_timer;
	timer->next = next;
...
static inline void python_ols_timer_remove(struct python_ols_timer *timer)
{
	struct python_ols_timer *next = timer->next;
	if (next)
...
python_ols_timer_cb(struct python_ols_timer *timer)
{
	return &((struct python_ols_callback *)timer)[-1];
}

static PyObject *timer_remove(PyObject *self, PyObject *args)
{
	struct ols_python_script *script = cur_python_script;
	PyObject *py_cb;
	...
	struct python_ols_callback *cb =
		find_python_ols_callback(script, py_cb);
	if (cb)
		remove_python_ols_callback(cb);
	return python_none();
}

static void timer_call(struct script_callback *p_cb)
{
	struct python_ols_callback *cb = (struct python_ols_callback *)p_cb;
...
{
	struct python_ols_callback *cb = p_cb;
	struct python_ols_timer *timer = python_ols_callback_extra_data(cb);
	python_ols_timer_init(timer);
}
...
static PyObject *timer_add(PyObject *self, PyObject *args)
{
	...
	struct python_ols_callback *cb = add_python_ols_callback_extra(
		script, py_cb, sizeof(struct python_ols_timer));
	struct python_ols_timer *timer = python_ols_callback_extra_data(cb);
...
static void ols_python_tick_callback(void *priv, float seconds)
{
	struct python_ols_callback *cb = priv;

	if (script_callback_removed(&cb->base)) {
		ols_remove_tick_callback(ols_python_tick_callback, cb);
		return;
	}
...
static PyObject *ols_python_remove_tick_callback(PyObject *self, PyObject *args)
{
	struct ols_python_script *script = cur_python_script;
	PyObject *py_cb = NULL;
...
static PyObject *ols_python_add_tick_callback(PyObject *self, PyObject *args)
{
	struct ols_python_script *script = cur_python_script;
	PyObject *py_cb = NULL;
	...
	struct python_ols_callback *cb = add_python_ols_callback(script, py_cb);
	ols_add_tick_callback(ols_python_tick_callback, cb);
	return python_none();
}
...
static void calldata_signal_callback(void *priv, calldata_t *cd)
{
	...
	if (libols_to_py(calldata_t, cd, false, &py_cd)) {
...
static PyObject *ols_python_signal_handler_disconnect(PyObject *self,
						      PyObject *args)
{
	struct ols_python_script *script = cur_python_script;
	PyObject *py_sh = NULL;
	...
	if (!py_to_libols(signal_handler_t, py_sh, &handler))
		return python_none();
	if (!py_cb || !PyFunction_Check(py_cb))
		return python_none();
	...
	while (cb) {
		...
		cb = find_next_python_ols_callback(script, cb, py_cb);
	}
...
static PyObject *ols_python_signal_handler_connect(PyObject *self,
						   PyObject *args)
{
	struct ols_python_script *script = cur_python_script;
	PyObject *py_sh = NULL;
...
static PyObject *ols_python_signal_handler_disconnect_global(PyObject *self,
							     PyObject *args)
{
	struct ols_python_script *script = cur_python_script;
	PyObject *py_sh = NULL;
...
static PyObject *ols_python_signal_handler_connect_global(PyObject *self,
							  PyObject *args)
{
	struct ols_python_script *script = cur_python_script;
	PyObject *py_sh = NULL;
...
{
	return PyLong_FromUnsignedLongLong(OLS_INVALID_HOTKEY_ID);
}
				...
				void *p_cb)
{
	...
	if (libols_to_py(ols_properties_t, props, false, &py_props) &&
	    libols_to_py(ols_property_t, p, false, &py_p)) {
...
static PyObject *properties_add_button(PyObject *self, PyObject *args)
{
	...
	if (!parse_args(args, "OssO", &py_props, &name, &text, &py_cb))
		return python_none();
	if (!py_to_libols(ols_properties_t, py_props, &props))
		return python_none();
	if (!py_cb || !PyFunction_Check(py_cb))
		return python_none();
	...
	if (!p || !libols_to_py(ols_property_t, p, false, &py_ret))
		return python_none();
			      ...
			      ols_property_t *p, ols_data_t *settings)
{
	...
	if (libols_to_py(ols_properties_t, props, false, &py_props) &&
	    libols_to_py(ols_property_t, p, false, &py_p) &&
	    libols_to_py(ols_data_t, settings, false, &py_settings)) {
...
static PyObject *property_set_modified_callback(PyObject *self, PyObject *args)
{
	...
	if (!parse_args(args, "OO", &py_p, &py_cb))
		return python_none();
	if (!py_to_libols(ols_property_t, py_p, &p))
		return python_none();
	if (!py_cb || !PyFunction_Check(py_cb))
		return python_none();
...
static PyObject *remove_current_callback(PyObject *self, PyObject *args)
{
	...
	if (cur_python_cb)
		remove_python_ols_callback(cur_python_cb);
	return python_none();
}
...
static PyObject *calldata_source(PyObject *self, PyObject *args)
{
	...
	if (!parse_args(args, "Os", &py_cd, &name))
		goto fail;
	if (!py_to_libols(calldata_t, py_cd, &cd))
		goto fail;

	ols_source_t *source = calldata_ptr(cd, name);
	libols_to_py(ols_source_t, source, false, &py_ret);
...
static PyObject *calldata_sceneitem(PyObject *self, PyObject *args)
{
	...
	ols_sceneitem_t *item = calldata_ptr(cd, name);
	libols_to_py(ols_sceneitem_t, item, false, &py_ret);
...
static bool enum_sources_proc(void *param, ols_source_t *source)
{
	...
	if (libols_to_py(ols_source_t, source, false, &py_source)) {
...
static PyObject *enum_sources(PyObject *self, PyObject *args)
{
	...
	ols_enum_sources(enum_sources_proc, list);
	return list;
}
			    ...
			    void *param)
{
	...
	if (libols_to_py(ols_sceneitem_t, item, false, &py_item)) {
...
static PyObject *scene_enum_items(PyObject *self, PyObject *args)
{
	...
	if (!parse_args(args, "O", &py_scene))
		return python_none();
	if (!py_to_libols(ols_scene_t, py_scene, &scene))
		return python_none();

	PyObject *list = PyList_New(0);
	ols_scene_enum_items(scene, enum_items_proc, list);
	return list;
}
...
static PyObject *sceneitem_group_enum_items(PyObject *self, PyObject *args)
{
	...
	if (!parse_args(args, "O", &py_sceneitem))
		return python_none();
	if (!py_to_libols(ols_sceneitem_t, py_sceneitem, &sceneitem))
		return python_none();

	PyObject *list = PyList_New(0);
	ols_sceneitem_group_enum_items(sceneitem, enum_items_proc, list);
	return list;
}
...
static PyObject *source_list_release(PyObject *self, PyObject *args)
{
	...
	for (Py_ssize_t i = 0; i < count; i++) {
		...
		if (py_to_libols(ols_source_t, py_source, &source)) {
...
static PyObject *sceneitem_list_release(PyObject *self, PyObject *args)
{
	...
	for (Py_ssize_t i = 0; i < count; i++) {
		...
		if (py_to_libols(ols_sceneitem_t, py_item, &item)) {
...
#define DEF_FUNC(n, c) {n, c, METH_VARARGS, NULL}
		...
		DEF_FUNC("ols_enum_sources", enum_sources),
		DEF_FUNC("ols_scene_enum_items", scene_enum_items),
		DEF_FUNC("ols_sceneitem_group_enum_items",
			 sceneitem_group_enum_items),
		DEF_FUNC("ols_remove_tick_callback",
			 ols_python_remove_tick_callback),
		DEF_FUNC("ols_add_tick_callback", ols_python_add_tick_callback),
		DEF_FUNC("signal_handler_disconnect",
			 ols_python_signal_handler_disconnect),
		DEF_FUNC("signal_handler_connect",
			 ols_python_signal_handler_connect),
		DEF_FUNC("signal_handler_disconnect_global",
			 ols_python_signal_handler_disconnect_global),
		DEF_FUNC("signal_handler_connect_global",
			 ols_python_signal_handler_connect_global),
		DEF_FUNC("ols_hotkey_unregister", hotkey_unregister),
		DEF_FUNC("ols_hotkey_register_frontend",
			 hotkey_register_frontend),
		DEF_FUNC("ols_properties_add_button", properties_add_button),
		DEF_FUNC("ols_property_set_modified_callback",
...
void ols_python_script_update(ols_script_t *script, ols_data_t *settings);
...
bool ols_python_script_load(ols_script_t *s)
{
	...
	if (python_loaded && !data->base.loaded) {
		...
		if (data->base.loaded) {
			blog(LOG_INFO,
			     "[ols-scripting]: Loaded python script: %s",
			     data->base.file.array);
			ols_python_script_update(s, NULL);
		}
	}
...
void ols_python_script_unload(ols_script_t *s)
{
	...
	if (data->p_prev_next_tick) {
		...
		struct ols_python_script *next = data->next_tick;
		if (next)
...
void ols_python_script_update(ols_script_t *s, ols_data_t *settings)
{
	struct ols_python_script *data = (struct ols_python_script *)s;
	...
	PyObject *py_settings;
	if (libols_to_py(ols_data_t, s->settings, false, &py_settings)) {
		PyObject *args = Py_BuildValue("(O)", py_settings);
...
ols_properties_t *ols_python_script_get_properties(ols_script_t *s)
{
	struct ols_python_script *data = (struct ols_python_script *)s;
	...
	if (!py_error())
		py_to_libols(ols_properties_t, ret, &props);
	Py_XDECREF(ret);
...
void ols_python_script_save(ols_script_t *s)
{
	struct ols_python_script *data = (struct ols_python_script *)s;
...
static void python_tick(void *param, float seconds)
{
	 ...
	 */
	struct ols_python_script *busy_script = NULL;
	bool valid;
	...
	pthread_mutex_lock(&timer_mutex);
	struct python_ols_timer *timer = first_timer;
	while (timer) {
		struct python_ols_timer *next = timer->next;
		struct python_ols_callback *cb = python_ols_timer_cb(timer);

		if (script_callback_removed(&cb->base)) {
			python_ols_timer_remove(timer);
		} else {
...
bool ols_scripting_load_python(const char *python_path)
{
	...
	add_python_frontend_funcs(py_olspython);
	py_error();
...
out:
	...
	if (python_loaded)
		ols_add_tick_callback(python_tick, NULL);
...
void ols_python_unload(void)
{
	...
	ols_remove_tick_callback(python_tick, NULL);
