				   ...
				   void *priv)
{
	...
	if (libols_to_py(ols_data_t, save_data, false, &py_save_data)) {
		PyObject *args = Py_BuildValue("(ON)", py_save_data,
					       PyBool_FromLong(saving));

		struct python_ols_callback *last_cb = cur_python_cb;
		cur_python_cb = cb;
...
#define DEF_FUNC(c) {"ols_frontend_" #c, c, METH_VARARGS, NULL}
