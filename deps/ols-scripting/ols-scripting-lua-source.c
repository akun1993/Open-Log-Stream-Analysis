			...
			int line)
{
	...
	int ret = SWIG_ConvertPtr(script, lua_idx, libols_out, info, 0);
	if (!SWIG_IsOK(ret)) {
		warn("%s:%d: SWIG failed to convert lua object to ols "
		     "object: %s%s%s",
...
#define ls_get_libols_obj(type, lua_index, ols_obj)                            \
	ls_get_libols_obj_(ls->script, #type " *", lua_index, ols_obj, ls->id, \
			 ...
			 int line)
{
	...
	SWIG_NewPointerObj(script, libols_in, info, (int)ownership);
	return true;
}

#define ls_push_libols_obj(type, ols_obj, ownership)                    \
	ls_push_libols_obj_(ls->script, #type " *", ols_obj, ownership, \
...
static void *ols_lua_source_create(ols_data_t *settings, ols_source_t *source)
{
	struct ols_lua_source *ls = ols_source_get_type_data(source);
	struct ols_lua_data *data = NULL;
	...
	ls_push_libols_obj(ols_data_t, settings, false);
	...
	if (data) {
		struct ols_lua_data *next = ls->first_source;
		data->next = next;
...
static void call_destroy(struct ols_lua_data *ld)
{
...
static uint32_t ols_lua_source_get_width(void *data)
{
	struct ols_lua_data *ld = data;
	struct ols_lua_source *ls = ld->ls;
	uint32_t width = 0;
...
static uint32_t ols_lua_source_get_height(void *data)
{
	struct ols_lua_data *ld = data;
	struct ols_lua_source *ls = ld->ls;
	uint32_t height = 0;
...
static void ols_lua_source_get_defaults(void *type_data, ols_data_t *settings)
{
	struct ols_lua_source *ls = type_data;
...
static ols_properties_t *ols_lua_source_get_properties(void *data)
{
	struct ols_lua_data *ld = data;
	struct ols_lua_source *ls = ld->ls;
...
static void ols_lua_source_update(void *data, ols_data_t *settings)
{
	struct ols_lua_data *ld = data;
	struct ols_lua_source *ls = ld->ls;
...
#define DEFINE_VOID_DATA_CALLBACK(name)               \
	static void ols_lua_source_##name(void *data) \
	{                                             \
		struct ols_lua_data *ld = data;       \
		struct ols_lua_source *ls = ld->ls;   \
		if (!have_func(name))                 \
...
static void ols_lua_source_video_tick(void *data, float seconds)
{
	struct ols_lua_data *ld = data;
	struct ols_lua_source *ls = ld->ls;
...
static void ols_lua_source_video_render(void *data, gs_effect_t *effect)
{
	struct ols_lua_data *ld = data;
	struct ols_lua_source *ls = ld->ls;
	...
	ls_push_data();
	ls_push_libols_obj(gs_effect_t, effect, false);
...
static void ols_lua_source_save(void *data, ols_data_t *settings)
{
	struct ols_lua_data *ld = data;
	struct ols_lua_source *ls = ld->ls;
...
static void ols_lua_source_load(void *data, ols_data_t *settings)
{
	struct ols_lua_data *ld = data;
	struct ols_lua_source *ls = ld->ls;
...
static void source_type_unload(struct ols_lua_source *ls)
{
#define unref(name)                                      \
...
static void ols_lua_source_free_type_data(void *type_data)
{
	struct ols_lua_source *ls = type_data;
...
EXPORT void ols_enable_source_type(const char *name, bool enable);

static inline struct ols_lua_source *find_existing(const char *id)
{
	struct ols_lua_source *existing = NULL;

	pthread_mutex_lock(&lua_source_def_mutex);
	struct ols_lua_source *ls = first_source_def;
	while (ls) {
...
static int ols_lua_register_source(lua_State *script)
{
	struct ols_lua_source ls = {0};
	struct ols_lua_source *existing = NULL;
	struct ols_lua_source *v = NULL;
	struct ols_source_info info = {0};
	const char *id;
	...
	info.id = v->id;
	info.type = (enum ols_source_type)get_table_int(script, -1, "type");
...
#undef get_callback
	...
	if (!existing) {
		...
		info.free_type_data = ols_lua_source_free_type_data;
		info.get_name = ols_lua_source_get_name;
		info.get_defaults2 = ols_lua_source_get_defaults;
		ols_register_source(&info);

		pthread_mutex_lock(&lua_source_def_mutex);
		v = info.type_data;

		struct ols_lua_source *next = first_source_def;
		v->next = next;
		if (next)
		...
		ols_enable_source_type(id, true);

		struct ols_lua_data *ld = v->first_source;
		while (ld) {
			struct ols_lua_source *ls = v;

			if (have_func(create)) {
				ols_source_t *source = ld->source;
				ols_data_t *settings =
...
void add_lua_source_functions(lua_State *script)
{
	...
	lua_pushstring(script, "ols_register_source");
	lua_pushcfunction(script, ols_lua_register_source);
	lua_rawset(script, -3);

	lua_pop(script, 1);
}

static inline void undef_source_type(struct ols_lua_script *data,
				     struct ols_lua_source *ls)
{
	pthread_mutex_lock(&ls->definition_mutex);
	pthread_mutex_lock(&data->mutex);

	ols_enable_source_type(ls->id, false);

	struct ols_lua_data *ld = ls->first_source;
	while (ld) {
		call_destroy(ld);
		ld = ld->next;
	}
...
void undef_lua_script_sources(struct ols_lua_script *data)
{
	pthread_mutex_lock(&lua_source_def_mutex);

	struct ols_lua_source *def = first_source_def;
	while (def) {
