...
{
	char **names = ols_frontend_get_scene_names();
	char **name = names;
	int i = 0;
...
{
	struct ols_frontend_source_list list = {0};
	ols_frontend_get_scenes(&list);

	lua_newtable(script);

	for (size_t i = 0; i < list.sources.num; i++) {
		ols_source_t *source = list.sources.array[i];
...
{
	ols_source_t *source = ols_frontend_get_current_scene();
	ls_push_libols_obj(ols_source_t, source, false);
	return 1;
}
...
{
	ols_source_t *source = NULL;
	ls_get_libols_obj(ols_source_t, 1, &source);
	ols_frontend_set_current_scene(source);
	return 0;
}
...
{
	struct ols_frontend_source_list list = {0};
	ols_frontend_get_transitions(&list);
...
{
	ols_source_t *source = ols_frontend_get_current_transition();
	ls_push_libols_obj(ols_source_t, source, false);
	return 1;
}
...
static int set_current_transition(lua_State *script)
{
	ols_source_t *source = NULL;
	ls_get_libols_obj(ols_source_t, 1, &source);
	ols_frontend_set_current_transition(source);
	return 0;
}

static int get_transition_duration(lua_State *script)
{
	int duration = ols_frontend_get_transition_duration();
...
static int set_transition_duration(lua_State *script)
{
	if (lua_isnumber(script, 1)) {
		...
		ols_frontend_set_transition_duration(duration);
	}
	return 0;
}
...
{
	char **names = ols_frontend_get_scene_collections();
	char **name = names;
	int i = 0;
...
{
	char *name = ols_frontend_get_current_scene_collection();
	lua_pushstring(script, name);
	bfree(name);
	return 1;
}
...
static int set_current_scene_collection(lua_State *script)
{
	if (lua_isstring(script, 1)) {
		...
		ols_frontend_set_current_scene_collection(name);
	}
	return 0;
}
...
{
	char **names = ols_frontend_get_profiles();
	char **name = names;
	int i = 0;
...
{
	char *name = ols_frontend_get_current_profile();
	lua_pushstring(script, name);
	bfree(name);
	return 1;
}
...
static int set_current_profile(lua_State *script)
{
	if (lua_isstring(script, 1)) {
		const char *name = lua_tostring(script, 1);
		ols_frontend_set_current_profile(name);
	}
	return 0;
}
...
static void frontend_event_callback(enum ols_frontend_event event, void *priv)
{
	struct lua_ols_callback *cb = priv;
	lua_State *script = cb->script;

	if (script_callback_removed(&cb->base)) {
		ols_frontend_remove_event_callback(frontend_event_callback, cb);
		return;
	}
...
{
	ols_frontend_add_event_callback(frontend_event_callback, cb);
}
...
static void frontend_save_callback(ols_data_t *save_data, bool saving,
				   void *priv)
{
	struct lua_ols_callback *cb = priv;
	lua_State *script = cb->script;
	...
	if (script_callback_removed(&cb->base)) {
		ols_frontend_remove_save_callback(frontend_save_callback, cb);
		return;
	}

	lock_callback();

	ls_push_libols_obj(ols_data_t, save_data, false);
...
{
	ols_frontend_add_save_callback(frontend_save_callback, cb);
}
...
static const ols_frontend_func functions[] = {
	{.name = "ols_frontend_get_scene_names", .function = get_scene_names},
	{.name = "ols_frontend_get_scenes", .function = get_scenes},
	{.name = "ols_frontend_get_current_scene",
	 .function = get_current_scene},
	{.name = "ols_frontend_set_current_scene",
	 .function = set_current_scene},
	{.name = "ols_frontend_get_transitions", .function = get_transitions},
	{.name = "ols_frontend_get_current_transition",
	 .function = get_current_transition},
	{.name = "ols_frontend_set_current_transition",
	 .function = set_current_transition},
	{.name = "ols_frontend_get_transition_duration",
	 .function = get_transition_duration},
	{.name = "ols_frontend_set_transition_duration",
	 .function = set_transition_duration},
	{.name = "ols_frontend_get_scene_collections",
	 .function = get_scene_collections},
	{.name = "ols_frontend_get_current_scene_collection",
	 .function = get_current_scene_collection},
	{.name = "ols_frontend_set_current_scene_collection",
	 .function = set_current_scene_collection},
	{.name = "ols_frontend_get_profiles", .function = get_profiles},
	{.name = "ols_frontend_get_current_profile",
	 .function = get_current_profile},
	{.name = "ols_frontend_set_current_profile",
	 .function = set_current_profile},
	{.name = "ols_frontend_remove_event_callback",
	 .function = remove_event_callback},
	{.name = "ols_frontend_add_event_callback",
	 .function = add_event_callback},
	{.name = "ols_frontend_remove_save_callback",
	 .function = remove_save_callback},
	{.name = "ols_frontend_add_save_callback",
...
void add_lua_frontend_funcs(lua_State *script)
{
	...
	size_t num_items = OLS_COUNTOF(functions);
