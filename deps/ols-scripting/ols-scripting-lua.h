...
struct lua_ols_callback;

extern THREAD_LOCAL struct lua_ols_callback *current_lua_cb;
...
#define lock_callback()                                                \
	struct ols_lua_script *__last_script = current_lua_script;     \
	struct lua_ols_callback *__last_callback = current_lua_cb;     \
	current_lua_cb = cb;                                           \
	current_lua_script = (struct ols_lua_script *)cb->base.script; \
...
add_lua_ols_callback_extra(lua_State *script, int stack_idx, size_t extra_size)
{
	struct ols_lua_script *data = current_lua_script;
	struct lua_ols_callback *cb =
		add_script_callback(&data->first_callback, (ols_script_t *)data,
...
static inline struct lua_ols_callback *add_lua_ols_callback(lua_State *script,
							    int stack_idx)
{
	return add_lua_ols_callback_extra(script, stack_idx, 0);
}

static inline void *lua_ols_callback_extra_data(struct lua_ols_callback *cb)
{
	return (void *)&cb[1];
}

static inline struct ols_lua_script *
lua_ols_callback_script(struct lua_ols_callback *cb)
{
	return (struct ols_lua_script *)cb->base.script;
}

static inline struct lua_ols_callback *
find_next_lua_ols_callback(lua_State *script, struct lua_ols_callback *cb,
			   int stack_idx)
{
	struct ols_lua_script *data = current_lua_script;

	cb = cb ? (struct lua_ols_callback *)cb->base.next
...
static inline struct lua_ols_callback *find_lua_ols_callback(lua_State *script,
							     int stack_idx)
{
	return find_next_lua_ols_callback(script, NULL, stack_idx);
}

static inline void remove_lua_ols_callback(struct lua_ols_callback *cb)
{
	remove_script_callback(&cb->base);
	luaL_unref(cb->script, LUA_REGISTRYINDEX, cb->reg_idx);
}

static inline void just_free_lua_ols_callback(struct lua_ols_callback *cb)
{
	just_free_script_callback(&cb->base);
}

static inline void free_lua_ols_callback(struct lua_ols_callback *cb)
{
