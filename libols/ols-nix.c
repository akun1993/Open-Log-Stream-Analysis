...
bool ols_hotkeys_platform_init(struct ols_core_hotkeys *hotkeys) {
  //   switch (ols_get_nix_platform()) {
  //   case OLS_NIX_PLATFORM_X11_EGL:
  //     hotkeys_vtable = ols_nix_x11_get_hotkeys_vtable();
  //     break;
  // #ifdef ENABLE_WAYLAND
  //   case ols_NIX_PLATFORM_WAYLAND:
  //     hotkeys_vtable = ols_nix_wayland_get_hotkeys_vtable();
  //     break;
  // #endif
  //   default:
  //     break;
...
void ols_key_to_str(ols_key_t key, struct dstr *dstr) {
  return hotkeys_vtable->key_to_str(key, dstr);
}

ols_key_t ols_key_from_virtual_key(int sym) {
  return hotkeys_vtable->key_from_virtual_key(sym);
}

int ols_key_to_virtual_key(ols_key_t key) {
  return hotkeys_vtable->key_to_virtual_key(key);
}

static inline void add_combo_key(ols_key_t key, struct dstr *str) {
  struct dstr key_str = {0};

  ols_key_to_str(key, &key_str);
...
void ols_key_combination_to_str(ols_key_combination_t combination,
                                struct dstr *str) {
  if ((combination.modifiers & INTERACT_CONTROL_KEY) != 0) {
    add_combo_key(OLS_KEY_CONTROL, str);
  }
  if ((combination.modifiers & INTERACT_COMMAND_KEY) != 0) {
    add_combo_key(OLS_KEY_META, str);
  }
  if ((combination.modifiers & INTERACT_ALT_KEY) != 0) {
    add_combo_key(OLS_KEY_ALT, str);
  }
  if ((combination.modifiers & INTERACT_SHIFT_KEY) != 0) {
    add_combo_key(OLS_KEY_SHIFT, str);
  }
  if (combination.key != OLS_KEY_NONE) {
