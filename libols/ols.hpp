...
using OLSDataArray =
    OLSRef<ols_data_array_t *, ols_data_array_addref, ols_data_array_release>;
using OLSOutput =
...
public:
  inline OLSRef() : OLSRefAutoRelease<T, release>::OLSRefAutoRelease(nullptr) {}
...
inline OLSWeakObject OLSGetWeakRef(ols_object_t *object) {
  return {ols_object_get_weak_object(object), OLSWeakObject::TakeOwnership()};
}
...
inline OLSWeakSource OLSGetWeakRef(ols_source_t *source) {
  return {ols_source_get_weak_source(source), OLSWeakSource::TakeOwnership()};
}
...
inline OLSWeakOutput OLSGetWeakRef(ols_output_t *output) {
  return {ols_output_get_weak_output(output), OLSWeakOutput::TakeOwnership()};
}
...
public:
  inline OLSSignal()
      : handler(nullptr), signal(nullptr), callback(nullptr), param(nullptr) {}

  inline OLSSignal(signal_handler_t *handler_, const char *signal_,
                   signal_callback_t callback_, void *param_)
      : handler(handler_), signal(signal_), callback(callback_), param(param_) {
  ...
  OLSSignal(OLSSignal &&other) noexcept
      : handler(other.handler), signal(other.signal), callback(other.callback),
        param(other.param) {
