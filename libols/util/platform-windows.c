...
void *os_dlopen(const char *path) {
  ...
  if (!h_library) {
    ...
    blog(LOG_INFO, "LoadLibrary failed for '%s': %s (%lu)", path, message,
         error);
...
void os_dlclose(void *module) { FreeLibrary(module); }
...
static bool has_qt5_import(VOID *base, PIMAGE_NT_HEADERS nt_headers) {
  __try {
    ...
    import = (PIMAGE_IMPORT_DESCRIPTOR)(
        (byte *)base + data_dir->VirtualAddress - section->VirtualAddress +
        section->PointerToRawData);

    while (import->Name != 0) {
      char *name =
          (char *)((byte *)base + import->Name - section->VirtualAddress +
                   section->PointerToRawData);
...
static bool has_ols_export(VOID *base, PIMAGE_NT_HEADERS nt_headers) {
  __try {
    ...
    export = (PIMAGE_EXPORT_DIRECTORY)((byte *)base + data_dir->VirtualAddress -
    ...
    for (j = 0; j < export->NumberOfNames; j++) {
      ...
      name = (CHAR *)base + names_ptr[j] - section->VirtualAddress +
             section->PointerToRawData;
...
void get_plugin_info(const char *path, bool *is_ols_plugin, bool *can_load) {
  ...
  *can_load = false;
  ...
  hFile = CreateFileW(wpath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
...
os_cpu_usage_info_t *os_cpu_usage_info_start(void) {
  ...
  GetProcessTimes(GetCurrentProcess(), &dummy, &dummy, &info->last_sys_time.ft,
                  &info->last_user_time.ft);
  ...
  if (os_get_abs_path(path, abs_path, 512) > 0) {
    if (os_utf8_to_wcs(abs_path, 0, w_abs_path, 512) > 0) {
      BOOL success = GetDiskFreeSpaceExW(
          w_abs_path, (PULARGE_INTEGER)&remainingSpace, NULL, NULL);
      if (success)
...
int os_rename(const char *old_path, const char *new_path) {
  ...
  code = MoveFileExW(old_path_utf16, new_path_utf16, MOVEFILE_REPLACE_EXISTING)
             ? 0
             : -1;
...
static bool initialize_version_functions(void) {
  ...
  get_file_version_info_size = (get_file_version_info_size_w_t)GetProcAddress(
      ver, "GetFileVersionInfoSizeW");
  get_file_version_info =
      (get_file_version_info_w_t)GetProcAddress(ver, "GetFileVersionInfoW");
...
static inline void get_reg_ver(struct win_version_info *ver) {
  ...
  const wchar_t *release_key =
      ver->build > 19041 ? L"DisplayVersion" : L"ReleaseId";
...
const char *get_win_release_id(void) { return win_release_id; }

uint32_t get_win_ver_int(void) { return get_winver(); }
...
void os_breakpoint(void) { __debugbreak(); }
...
char *os_generate_uuid(void) {
  ...
  dstr_printf(&uuid_str, "%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x",
