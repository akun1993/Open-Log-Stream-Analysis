...
#define FREE_OLS_HASH_TABLE(handle, table, type)                            \
	do {                                                                \
		...
		int unfreed = 0;                                            \
		HASH_ITER (handle, *(struct ols_context_data **)table, ctx, \
			   tmp) {                                           \
...
static inline bool ols_init_hotkeys(void)
{
	struct ols_core_hotkeys *hotkeys = &ols->hotkeys;
	bool success = false;
	...
	hotkeys->signals = ols->signals;
	hotkeys->name_map_init_token = ols_pthread_once_init_token;


	if (!ols_hotkeys_platform_init(hotkeys))
		return false;
	...
	if (os_event_init(&hotkeys->stop_event, OS_EVENT_TYPE_MANUAL) != 0)
		goto fail;
	if (pthread_create(&hotkeys->hotkey_thread, NULL, ols_hotkey_thread,
			   NULL))
		goto fail;
...
static inline void ols_free_hotkeys(void)
{
	struct ols_core_hotkeys *hotkeys = &ols->hotkeys;

	ols_hotkey_name_map_free();

	ols_hotkeys_platform_free(hotkeys);
...
char *ols_find_data_file(const char *file)
{
	...
	blog(LOG_ERROR, "Failed to find file '%s' in libols data directory",
	     file);
...
bool ols_initialized(void)
{
	return ols != NULL;
}

uint32_t ols_get_version(void)
{
	return LIBOLS_API_VER;
}

const char *ols_get_version_string(void)
{
	return OLS_VERSION;
}
...
const char *ols_get_locale(void)
{
	return ols->locale;
}

#define OLS_SIZE_MIN 2
#define OLS_SIZE_MAX (32 * 1024)

static inline bool size_valid(uint32_t width, uint32_t height)
{
	return (width >= OLS_SIZE_MIN && height >= OLS_SIZE_MIN &&
		width <= OLS_SIZE_MAX && height <= OLS_SIZE_MAX);
}
...
{
	if (idx >= ols->filter_types.num)
		return false;
	*id = ols->filter_types.array[idx].id;
	return true;
}
...
void ols_enum_all_sources(bool (*enum_proc)(void *, ols_source_t *),
			  void *param)
{
	ols_enum_uuid(&ols->data.sources, &ols->data.sources_mutex, enum_proc,
		      param);
}

void ols_enum_outputs(bool (*enum_proc)(void *, ols_output_t *), void *param)
{
	ols_enum(&ols->data.first_output, &ols->data.outputs_mutex, enum_proc,
		 param);
}
...
signal_handler_t *ols_get_signal_handler(void)
{
	return ols->signals;
}

proc_handler_t *ols_get_proc_handler(void)
{
	return ols->procs;
}
					  ...
					  bool is_private)
{
	...
	source->private_settings =
		ols_data_get_obj(source_data, "private_settings");
...
ols_data_t *ols_save_source(ols_source_t *source)
{
	...
	ols_data_set_obj(source_data, "private_settings",
			 source->private_settings);
...
void ols_reset_source_uuids()
{
	...
	struct ols_context_data *ht =
		(struct ols_context_data *)ols->data.sources;
			   ...
			   ols_data_t *hotkey_data, bool private)
{
	...
	context->hotkey_data = ols_data_newref(hotkey_data);
	return true;
}
...
{
	ols_hotkeys_context_release(context);
