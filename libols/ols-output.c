...
{
	if (!ols_context_data_init(&output->context, OLS_OBJ_TYPE_OUTPUT,
...
{
	return ols_output_valid(output, "ols_output_get_name")
		       ? output->context.name
		       : NULL;
}
...
{
	return ols_output_valid(output, "ols_output_get_flags")
		       ? output->info.flags
		       : 0;
}
...
static inline void signal_stop(struct ols_output *output)
{
	...
	calldata_set_string(&params, "last_error",
			    ols_output_get_last_error(output));
...
{
	return ols_output_valid(output, "ols_output_get_id") ? output->info.id
							     : NULL;
}
