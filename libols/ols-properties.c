...
void ols_properties_remove_by_name(ols_properties_t *props, const char *name) {
  ...
  HASH_ITER(hh, props->properties, cur, tmp) {
    ...
    ols_properties_remove_by_name(ols_property_group_content(cur), name);
  }
}
...
void ols_properties_apply_settings_internal(
  ...
  while (p) {
    if (p->type == OLS_PROPERTY_GROUP) {
      ols_properties_apply_settings_internal(ols_property_group_content(p),
...
void ols_properties_apply_settings(ols_properties_t *props,
  ...
  ols_properties_apply_settings_internal(props, &properties_with_callback);

  while (properties_with_callback.num > 0) {
    struct ols_property *p =
        *(struct ols_property **)da_end(properties_with_callback);
    if (p->modified)
...
static inline bool contains_prop(struct ols_properties *props,
                                 const char *name) {
  struct ols_property *p, *tmp;
...
static ols_property_t *add_flt(ols_properties_t *props, const char *name,
  ...
  struct ols_property *p = new_prop(props, name, desc, OLS_PROPERTY_FLOAT);
...
ols_property_t *ols_properties_add_list(ols_properties_t *props,
  ...
  if (type == OLS_COMBO_TYPE_EDITABLE && format != OLS_COMBO_FORMAT_STRING) {
    blog(LOG_WARNING,
...
ols_property_t *ols_properties_add_button(ols_properties_t *props,
  ...
  struct ols_property *p = new_prop(props, name, text, OLS_PROPERTY_BUTTON);
...
get_list_fmt_data(struct ols_property *p, enum ols_combo_format format) {
...
void ols_property_set_long_description(ols_property_t *p,
...
const char *ols_property_name(ols_property_t *p) { return p ? p->name : NULL; }
...
bool ols_property_enabled(ols_property_t *p) { return p ? p->enabled : false; }

bool ols_property_visible(ols_property_t *p) { return p ? p->visible : false; }
...
void ols_property_int_set_limits(ols_property_t *p, int min, int max,
                                 int step) {
  struct int_data *data = get_type_data(p, OLS_PROPERTY_INT);
  if (!data)
    return;
...
void ols_property_button_set_type(ols_property_t *p,
                                  enum ols_button_type type) {
  struct button_data *data = get_type_data(p, OLS_PROPERTY_BUTTON);
  if (!data)
    return;
...
size_t ols_property_list_add_bool(ols_property_t *p, const char *name,
                                  bool val) {
  struct list_data *data = get_list_data(p);
