...
static void load_all_callback(void *param,
                              const struct ols_module_info2 *info) {
...
static char *make_data_directory(const char *module_name,
                                 const char *data_dir) {
  ...
  if (os_glob(search_path.array, 0, &gi) == 0) {
    for (size_t i = 0; i < gi->gl_pathc; i++) {
      if (search_directories == gi->gl_pathv[i].directory)
        process_found_module(omp, gi->gl_pathv[i].path, search_directories,
                             callback, param);
    }
...
lookup_t *ols_module_load_locale(ols_module_t *module,
  ...
  if (!text_lookup_add(lookup, file))
    blog(LOG_WARNING, "Failed to load '%s' text for module: '%s'", locale,
         module->file);
