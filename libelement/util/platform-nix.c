...
void *os_dlopen(const char *path) {
  ...
  if (!dstr_find(&dylib_name, ".framework") &&
      !dstr_find(&dylib_name, ".plugin") && !dstr_find(&dylib_name, ".dylib") &&
      !dstr_find(&dylib_name, ".so"))
#else
...
void *os_dlsym(void *module, const char *func) { return dlsym(module, func); }
...
void os_sleep_ms(uint32_t duration) { usleep(duration * 1000); }
...
bool os_get_emulation_status(void) { return false; }

#endif

bool os_file_exists(const char *path) { return access(path, F_OK) == 0; }
...
int os_rmdir(const char *path) { return rmdir(path); }
...
char *os_getcwd(char *path, size_t size) { return getcwd(path, size); }
...
static void reset_screensaver(os_inhibit_t *info) {
  ...
  int err =
      posix_spawnp(&pid, "xdg-screensaver", NULL, &info->attr, argv, environ);
  if (err == 0) {
    int status;
...
bool os_inhibit_sleep_set_active(os_inhibit_t *info, bool active) {
  ...
  if (active) {
    ret = pthread_create(&info->screensaver_thread, NULL, &screensaver_thread,
                         info);
    if (ret < 0) {
...
void os_breakpoint() { raise(SIGTRAP); }
