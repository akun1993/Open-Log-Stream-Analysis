#ifndef __OLS_PAD_HPP__
#define __OLS_PAD_HPP__

#include <stdint.h>
#include <OlsBuffer.hpp>
 ...
 */
typedef OlsFlowReturn (*ols_pad_chain_function)( OlsElement *parent,
                                                OlsBuffer *buffer);
 ...
 * Returns: #Ols_FLOW_OK for success
 */
typedef OlsFlowReturn (*ols_pad_chain_list_function)( OlsElement *parent,
 ...
 */
typedef bool (*ols_pad_event_function)( OlsElement *parent,OlsEvent *event);
 ...
 * Since: 1.8
 */
typedef OlsFlowReturn (*ols_pad_event_full_function)(OlsElement *parent,OlsEvent *event);
 ...
 */
typedef OlsPadLinkReturn (*ols_pad_link_function)(OlsElement *parent,OlsPad *peer);
/**
 ...
 */
typedef void (*ols_pad_unlink_function)(OlsPad *pad, OlsElement *parent);
...
public:
  ...
  void setChainFunctionFull(ols_pad_chain_function chain,void *user_data);
  void setChainListFunctionFull(ols_pad_chain_list_function chainlist,void *user_data);
  void setLinkFunctionFull(ols_pad_link_function link,void *user_data);
  void setUnlinkFunctionFull(ols_pad_unlink_function unlink,void *user_data);
  bool unlink( OlsPad *sinkpad) ;
  bool isLinked();
  OlsPadLinkReturn linkFull( OlsPad *sinkpad);
  OlsPadLinkReturn link( OlsPad *sinkpad);
  OlsFlowReturn chainDataUnchecked(OlsPadProbeType type, void *data);
  OlsFlowReturn pushData( OlsPadProbeType type,void *data);
  OlsFlowReturn push( OlsBuffer *buffer);
  OlsFlowReturn pushEventUnchecked(OlsEvent *event, OlsPadProbeType type);
  OlsFlowReturn sendEventUnchecked( OlsEvent *event,OlsPadProbeType type);
  bool pushEvent(OlsEvent *event);

  OlsPad *getPeer();

  //OlsFlowReturn chainListDefault(ols_object_t *parent,ols_buffer_list_t *list) ;
  // OlsFlowReturn pushData(OlsPadProbeType type, void *data);
  // OlsFlowReturn push( ols_buffer_t *buffer);
  ...
  OLSPadDirection m_direction;

  pthread_mutex_t m_stream_rec_lock;

  /* block cond, mutex is from the object */
  pthread_cond_t m_block_cond;

  OLSPadMode m_mode;

  /* pad link */
  OlsPad *m_peer;
  ols_pad_link_function m_linkfunc;
  void *m_linkdata;

  ols_pad_unlink_function m_unlinkfunc;
  void *m_unlinkdata;

  /* data transport functions */
  ols_pad_chain_function m_chainfunc;
  void *chaindata;

  ols_pad_chain_list_function m_chainlistfunc;
  void *m_chainlistdata;

  ols_pad_event_function m_eventfunc;
  void *m_eventdata;

  OlsFlowReturn m_last_flowret;
  ols_pad_event_full_function m_eventfullfunc;
