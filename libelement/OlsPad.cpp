 ...
 */
#define OLS_PAD_GET_LOCK(obj) (OLS_PAD_CAST(obj)->mutex)
/**
 * OLS_PAD_LOCK:
...
static bool ols_pad_event_default(ols_pad_t *pad, ols_object_t *parent,
 ...
 OlsPad::OlsPad() 
 {
  ...
  pthread_cond_init(&m_block_cond, NULL);

  pthread_mutex_init(&m_mutex, NULL);

  pthread_mutex_init_recursive(&m_stream_rec_lock);
 ...
 */
void OlsPad::setChainFunctionFull(ols_pad_chain_function chain,void *user_data) {

  m_chainfunc = chain;
  m_chaindata = user_data;
 ...
 */
void OlsPad::setChainListFunctionFull(ols_pad_chain_list_function chainlist,
                                          void *user_data) {

  m_chainlistfunc = chainlist;
  m_chainlistdata = user_data;
 ...
 */
void OlsPad::setLinkFunctionFull(ols_pad_link_function link,
                                    void *user_data) {
  m_linkfunc = link;
  m_linkdata = user_data;
}
 ...
 */
void OlsPad::setUnlinkFunctionFull(ols_pad_unlink_function unlink,
                                      void *user_data) {
 ...
 * MT safe.
 */
bool OlsPad::unlink( OlsPad *sinkpad) {
  bool result = false;
 ...
 *          what went wrong.
 */
OlsPadLinkReturn OlsPad::linkFull( OlsPad *sinkpad) {
 ...
 * MT Safe.
 */
OlsPadLinkReturn OlsPad::link( OlsPad *sinkpad) {
  return linkFull( sinkpad);
}
 ...
 */
 OlsFlowReturn OlsPad::chainDataUnchecked( OlsPadProbeType type, void *data) {
  OlsFlowReturn ret;
  ols_object_t *parent;
  bool handled = false;
...
OlsFlowReturn OlsPad::chainListDefault( OlsElement *parent,
                                                ols_buffer_list_t *list) {
...
static OlsFlowReturn OlsPad::pushData(OlsPadProbeType type, void *data) {
  ols_pad_t *peer;
  OlsFlowReturn ret;
  bool handled = false;
 ...
 * MT safe.
 */
OlsFlowReturn OlsPad::push(OlsBuffer *buffer) {
  OlsFlowReturn res;
  ...
  res = pushData(OLS_PAD_PROBE_TYPE_BUFFER | OLS_PAD_PROBE_TYPE_PUSH, buffer);
 ...
 * MT safe.
 */
OlsPad *OlsPad::getPeer() {
  return m_peer;
}

static OlsFlowReturn OlsPad::sendEventUnchecked(ols_pad_t *pad,
                                                  OlsEvent *event,
                                                  OlsPadProbeType type) {
...
/* should be called with pad LOCK */
 OlsFlowReturn OlsPad::pushEventUnchecked( OlsEvent *event, OlsPadProbeType type) {
  OlsFlowReturn ret;
  OlsPad *peerpad = m_peer;
 ...
 // peerpad = OLS_PAD_PEER(pad);
  ...
  ret = peerpad->sendEventUnchecked( event, type);
 ...
 * MT safe.
 */
bool OlsPad::pushEvent( OlsEvent *event) {
  bool res = false;
