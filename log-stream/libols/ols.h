...
#define OLS_OBJECT_UNLOCK(obj) pthread_mutex_unlock(OLS_OBJECT_GET_LOCK(obj))
  ...
  typedef void (*ols_ev_task_t)(void *param);

  enum ols_ev_task_type
  {
    OLS_TASK_UI,
    OLS_TASK_DESTROY,
  };

  EXPORT void ols_queue_task(enum ols_ev_task_type type, ols_ev_task_t task,
                             void *param, bool wait);
  EXPORT bool ols_in_task_thread(enum ols_ev_task_type type);

  EXPORT bool ols_wait_for_destroy_queue(void);

  typedef void (*ols_ev_task_handler_t)(ols_ev_task_t task, void *param, bool wait);
  EXPORT void ols_set_ui_task_handler(ols_ev_task_handler_t handler);
...
#define OLS_SOURCE_FLAG_FORCE_MONO (1 << 1)
  ...
  /** Updates settings for this source */
  EXPORT void ols_process_update(ols_process_t *source, ols_data_t *settings);
