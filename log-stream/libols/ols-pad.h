 ...
 * @pad: a #GstPad
 *
 * Get the #os_task_info_t of @pad. Accessor macro used by OLStream. Use the
 * ols_pad_start_task(), gst_pad_stop_task() and ols_pad_pause_task()
 * functions instead.
 */
#define OLS_PAD_TASK(pad)		(OLS_PAD_CAST(pad)->task)



#define OLS_PAD_GET_STREAM_LOCK(pad) (&OLS_PAD_CAST(pad)->stream_rec_lock)


#define OLS_PAD_STREAM_LOCK(pad)  (pthread_mutex_lock(OLS_PAD_GET_STREAM_LOCK(pad)))
#define OLS_PAD_STREAM_UNLOCK(pad) (pthread_mutex_unlock(OLS_PAD_GET_STREAM_LOCK(pad)))
