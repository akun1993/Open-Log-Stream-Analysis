 ...
 * ols_task_set_lock().
 *
 * The task can be started, paused and stopped with ols_task_start(), ols_task_pause()
 * and ols_task_stop() respectively or with the ols_task_set_state() function.
 *
 ...
 *
 * Stopping a task with ols_task_stop() will not immediately make sure the task is
 * not running anymore. Use ols_task_join() to make sure the task is completely
 * stopped and the thread is stopped.
 *
 * After creating a #GstTask, use ols_object_unref() to free its resources. This can
...
#define SET_TASK_STATE(t,s) (os_atomic_set_long (&OLS_TASK_STATE(t), (s)))
#define GET_TASK_STATE(t)   ((OlsTaskState) os_atomic_load_long (&OLS_TASK_STATE(t)))
  ...
  */
 ols_task_t *
 ols_task_new (ols_task_function func, void * user_data)
 {
   ols_task_t *task;
  ...
  */
 OlsTaskState
 ols_task_get_state (ols_task_t * task)
 {
    OlsTaskState result;
 ...
 static bool start_task (ols_task_t * task)
 {
  ...
  //  priv->id =
  //      ols_task_pool_push (priv->pool_id, (GstTaskPoolFunction) ols_task_func,
  //      task, &error);
  ...
  */
 bool
 ols_task_pause (ols_task_t * task)
 {
