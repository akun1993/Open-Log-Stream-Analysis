...
static inline void ols_data_add_json_object_data(ols_data_t *data,
                                                 json_t *jobj) {
  const char *item_key;
  json_t *jitem;
...
bool ols_data_save_json_safe(ols_data_t *data, const char *file,
  ...
  if (json && *json) {
    return os_quick_write_utf8_file_safe(file, json, strlen(json), false,
                                         temp_ext, backup_ext);
  }
...
ols_data_t *ols_data_get_defaults(ols_data_t *data) {
  ...
  HASH_ITER(hh, data->items, item, temp) {
    ...
    case OLS_DATA_STRING: {
      const char *str = ols_data_get_default_string(data, name);
    ...
    case OLS_DATA_NUMBER: {
      ...
      case OLS_DATA_NUM_DOUBLE: {
        double val = ols_data_get_default_double(data, name);
        ols_data_set_double(defaults, name, val);
        break;
      }

      case OLS_DATA_NUM_INT: {
        long long val = ols_data_get_default_int(data, name);
    ...
    case OLS_DATA_ARRAY: {
      ols_data_array_t *arr = ols_data_get_default_array(data, name);
      ols_data_array_t *defs = ols_data_array_create();

      ols_data_array_enum(arr, get_defaults_array_cb, (void *)defs);
...
static void set_item_data(struct ols_data *data, struct ols_data_item **item,
  ...
  if ((!item || !*item) && data) {
    new_item = ols_data_item_create(name, ptr, size, type, default_data,
                                    autoselect_data);
...
static void
copy_obj(struct ols_data *data, const char *name, struct ols_data *obj,
...
static inline void copy_item(struct ols_data *data,
                             struct ols_data_item *item) {
  const char *name = get_item_name(item);
...
void ols_data_apply(ols_data_t *target, ols_data_t *apply_data) {
  ...
  HASH_ITER(hh, apply_data->items, item, temp) { copy_item(target, item); }
}
...
static inline void ols_set_obj(ols_data_t *data, ols_data_item_t **item,
                               ...
                               set_item_t set_item_) {
  set_item_(data, item, name, &obj, sizeof(ols_data_t *), OLS_DATA_OBJECT);
}

static inline void ols_set_array(ols_data_t *data, ols_data_item_t **item,
                                 const char *name, ols_data_array_t *array,
                                 set_item_t set_item_) {
  set_item_(data, item, name, &array, sizeof(ols_data_t *), OLS_DATA_ARRAY);
}
...
void ols_data_set_default_int(ols_data_t *data, const char *name,
                              long long val) {
  ols_set_int(data, NULL, name, val, set_item_def);
}

void ols_data_set_default_double(ols_data_t *data, const char *name,
                                 double val) {
...
void ols_data_set_autoselect_bool(ols_data_t *data, const char *name,
                                  bool val) {
...
ols_data_array_t *ols_data_get_default_array(ols_data_t *data,
                                             const char *name) {
...
void ols_data_array_insert(ols_data_array_t *array, size_t idx,
                           ols_data_t *obj) {
  if (!array || !obj)
    return;
...
static inline long long item_int(struct ols_data_item *item,
  ...
  if (item && (num = get_data(item))) {
    return (num->type == OLS_DATA_NUM_INT) ? num->int_val
...
static inline long long data_item_get_int(ols_data_item_t *item,
                                          get_data_t get_data) {
  return item_int(item_valid(item, OLS_DATA_NUMBER) ? item : NULL, get_data);
}
...
static inline double data_item_get_double(ols_data_item_t *item,
                                          get_data_t get_data) {
  return item_double(item_valid(item, OLS_DATA_NUMBER) ? item : NULL, get_data);
}
...
static inline bool data_item_get_bool(ols_data_item_t *item,
  ...
  return item_valid(item, OLS_DATA_BOOLEAN) && (data = get_data(item)) ? *data
                                                                       : false;
}
...
static inline ols_data_t *data_item_get_obj(ols_data_item_t *item,
                                            get_obj_t get_obj) {
  ols_data_t *obj = item_valid(item, OLS_DATA_OBJECT) ? get_obj(item) : NULL;
