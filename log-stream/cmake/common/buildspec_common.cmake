# OLS common build dependencies module
...
# _check_deps_version: Checks for ols-deps VERSION file in prefix paths
function(_check_deps_version version)
  ...
  foreach(path IN LISTS CMAKE_PREFIX_PATH)
    if(EXISTS "${path}/share/ols-deps/VERSION")
      ...
      file(READ "${path}/share/ols-deps/VERSION" _check_version)
...
# _check_dependencies: Fetch and extract pre-built OLS build dependencies
  ...
  foreach(dependency IN LISTS dependencies_list)
    ...
    if(EXISTS "${dependencies_dir}/.dependency_${dependency}_${arch}.sha256")
      file(READ "${dependencies_dir}/.dependency_${dependency}_${arch}.sha256"
           OLS_DEPENDENCY_${dependency}_${arch}_HASH)
    endif()

    set(skip FALSE)
    if(dependency STREQUAL prebuilt OR dependency STREQUAL qt6)
      if(OLS_DEPENDENCY_${dependency}_${arch}_HASH STREQUAL ${hash})
    ...
    elseif(dependency STREQUAL cef)
      if(NOT ENABLE_BROWSER)
        set(skip TRUE)
      elseif(OLS_DEPENDENCY_${dependency}_${arch}_HASH STREQUAL ${hash} AND (CEF_ROOT_DIR AND EXISTS "${CEF_ROOT_DIR}"))
        set(skip TRUE)
      endif()
    endif()
