...
set(OLS_STANDALONE_PLUGIN_DIR ${CMAKE_SOURCE_DIR}/release)
...
# Old-Style plugin detected, find "modern" libols variant instead and set global
# include directories to fix "bad" plugin behavior
if(DEFINED LIBOLS_INCLUDE_DIR AND NOT TARGET OLS::libols)
  message(
    DEPRECATION
      "You are using an outdated method of adding 'libols' to your project. Refer to the updated wiki on how to build and export 'libols' and use it in your plugin projects."
  )
  find_package(libols REQUIRED)
  if(TARGET OLS::libols)
    set_target_properties(OLS::libols PROPERTIES IMPORTED_GLOBAL TRUE)
    message(STATUS "OLS: Using modern libols target")

    add_library(libols ALIAS OLS::libols)
    if(OS_WINDOWS)
      add_library(w32-pthreads ALIAS OLS::w32-pthreads)
    endif()
  endif()
endif()
...
if(CMAKE_INSTALL_PREFIX_INITIALIZED_TO_DEFAULT AND (OS_WINDOWS OR OS_MACOS))
  set(CMAKE_INSTALL_PREFIX
      ${OLS_STANDALONE_PLUGIN_DIR}
      CACHE STRING "Directory to install OLS plugin after building" FORCE)
endif()
...
if(NOT CMAKE_BUILD_TYPE)
  set(CMAKE_BUILD_TYPE
      "RelWithDebInfo"
      CACHE STRING
            "OLS build type [Release, RelWithDebInfo, Debug, MinSizeRel]" FORCE)
...
if(NOT QT_VERSION)
  set(QT_VERSION
      AUTO
      CACHE STRING "OLS Qt version [AUTO, 6, 5]" FORCE)
...
if(OS_MACOS)
  ...
  if(CMAKE_INSTALL_PREFIX_INITIALIZED_TO_DEFAULT)
    set(CMAKE_INSTALL_PREFIX
        ...
        CACHE STRING "Directory to install OLS to after building" FORCE)
  endif()
  ...
  if(NOT OLS_BUNDLE_CODESIGN_TEAM)
    ...
    if(NOT XCODE)
      ...
      unset(OLS_BUNDLE_CODESIGN_IDENTITY)
      set_property(CACHE OLS_BUNDLE_CODESIGN_TEAM
                   PROPERTY HELPSTRING "OLS code signing team for macOS")
  ...
  # match the host binary, in this case OLS.app.
  ...
  # Enable linker codesigning by default. Building OLS or plugins on host
  ...
  function(setup_plugin_target target)
    ...
    if(NOT XCODE)
      set(_COMMAND
          ...
          --sign \\\"${OLS_BUNDLE_CODESIGN_IDENTITY}\\\" \\
          --options runtime \\
  ...
  if(OS_POSIX)
    ...
    if(NOT LINUX_PORTABLE)
      ...
      set(CMAKE_INSTALL_RPATH
          "$ORIGIN/" "${CMAKE_INSTALL_PREFIX}/${OLS_LIBRARY_DESTINATION}")
    ...
    if(OS_LINUX)
      ...
      set(CPACK_DEBIAN_PACKAGE_DEPENDS
          "ols-studio (>= 27.0.0), libqt5core5a (>= 5.9.0~beta), libqt5gui5 (>= 5.3.0), libqt5widgets5 (>= 5.7.0)"
      )
  ...
  function(setup_plugin_target target)
    ...
    install(
      ...
      DESTINATION $<CONFIG>/${OLS_PLUGIN_DESTINATION}
    ...
    # Set up plugin for testing in available OLS build on Windows
    if(OS_WINDOWS AND DEFINED OLS_BUILD_DIR)
      setup_target_for_testing(${target} ols-plugins/${target})
    endif()
    ...
    add_custom_command(
      ...
      POST_BUILD
      COMMAND
        "${CMAKE_COMMAND}" -DCMAKE_INSTALL_PREFIX=${OLS_OUTPUT_DIR}
        -DCMAKE_INSTALL_COMPONENT=ols_rundir
  ...
  function(setup_target_resources target destination)
    if(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/data)
      ...
      install(
        DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}/data
        DESTINATION $<CONFIG>/${OLS_DATA_DESTINATION}/${destination}
        USE_SOURCE_PERMISSIONS
        COMPONENT ols_rundir
        EXCLUDE_FROM_ALL)
    endif()
  ...
  if(OS_WINDOWS)
    # Additional Windows-only helper function to copy plugin to existing OLS
    # development directory:
    #
    # Copies plugin with associated PDB symbol files as well as contents of data
    # directory into the OLS rundir as specified by "OLS_BUILD_DIR".
    function(setup_target_for_testing target destination)
      install(
        FILES $<TARGET_FILE:${target}>
        DESTINATION $<CONFIG>/${OLS_PLUGIN_DESTINATION}
        COMPONENT ols_testing
        EXCLUDE_FROM_ALL)
      ...
      add_custom_command(
        ...
        POST_BUILD
        COMMAND
          "${CMAKE_COMMAND}" -DCMAKE_INSTALL_PREFIX=${OLS_BUILD_DIR}/rundir
          -DCMAKE_INSTALL_COMPONENT=ols_testing
          -DCMAKE_INSTALL_CONFIG_NAME=$<CONFIG> -P
          ${CMAKE_CURRENT_BINARY_DIR}/cmake_install.cmake
        COMMENT "Installing to OLS test directory"
        VERBATIM)
    endfunction()
  endif()
endif()
