...
void FlowChartScene::loadFlowChart(const QJsonObject &json)
{
    ...
    if (m_nodes.contains("开始"))
    {
        NodeState currentStartState = mapJsonStatusToNodeState(
            json["states"].toObject()["开始"].toObject()["status"].toString());
...
void FlowChartScene::createLegend()
{
    ...
    QGraphicsTextItem *title = new QGraphicsTextItem("状态图例");
    ...
    QVector<QPair<QString, QColor>> legendItems = {
        {"运行中", QColor(50, 200, 50)},
        {"等待中", QColor(255, 165, 0)},
        {"已运行", QColor(65, 105, 225)},
        {"未运行", QColor(120, 120, 120)},
        {"跳过", QColor(148, 0, 211)},
        {"失败", QColor(220, 20, 60)}};
...
void FlowChartScene::calculateNodeLevels()
{
    ...
    do
    {
        ...
        for (FlowEdge *edge : m_edges)
        {
            ...
            if (!m_nodeLevels.contains(source) || !m_nodeLevels.contains(target) || target == "开始")
                continue;
        ...
        for (const QString &nodeId : nodes)
        {
            ...
            for (FlowEdge *edge : m_edges)
            {
                if (edge->sourceNode()->id() == nodeId)
                {
                    ...
                    if (targetNode && node->x() > targetNode->x() && targetNode->id() != "开始")
